...
int main(void) {
	...
	Buffer greenDotBuffer(attrSizes, gd, sizeof(gd));
	...
	while (!glfwWindowShouldClose(window)) {
		...
		ImGui::Text("avgFuel: %6.2f", (float)getReferenceTemperature(cur_fg));
		ImGui::Text("avgExh: %6.2f", (float)getReferenceTemperature(cur_eg));
		ImGui::SliderInt("speed", &speed, 1, 10);
		ImGui::Checkbox("recordFrames", &recordFrames);
		ImGui::Checkbox("sourceType: Fuel", &sourceType);
		ImGui::SliderFloat("dt", &dt, 0.0001f, 0.1f);
		ImGui::SliderFloat("dx", &dx, 0.01f, 5.0f);
		ImGui::SliderFloat("nv", &nv, 0.f, 20.f);
		ImGui::SliderFloat("kappa", &kappa, 0.f, 1.f);
		ImGui::SliderFloat("kappa_fuel", &kappa_fuel, 0.f, 1.f);
		ImGui::SliderFloat("kappa_exhaust", &kappa_exhaust, 0.f, 1.f);
		ImGui::SliderFloat("dissipation", &dissipation, 0.f, 1.f);
		ImGui::SliderFloat("dissipation_fuel", &dissipation_fuel, 0.f, 1.f);
		ImGui::SliderFloat("dissipation_exhaust", &dissipation_exhaust, 0.f, 1.f);
		ImGui::SliderFloat("buoyancy", &buoyancy, 0.f, 10.f);
		ImGui::SliderFloat("gravity", &gravity, 0.f, 5.f);
		ImGui::SliderFloat("reactionTemperature", &reactionTemperature, 0.f, 500.f);
		ImGui::SliderFloat("expansion", &expansion, 0.f, 5.f);
		ImGui::SliderFloat("pctFuelBurn", &pctFuelBurn, 0.f, 1.f);
		ImGui::SliderFloat("stochiometricMixture", &stochiometricMixture, 0.f, 10.f);
		ImGui::SliderFloat("combustionHeat", &combustionHeat, 0.f, 500.f);
		ImGui::SliderFloat("vorticityStrength", &vorticityStrength, 0.f, 10.f);
		ImGui::SliderFloat("srcTemp", &srcTemp, -500.f, 5000.f);
		...
		if (ImGui::Button("Fuel")) {
			vizType = 1;
		}	ImGui::SameLine();
		if(ImGui::Button("Fire")) {
			vizType = 2;
		}
		ImGui::SliderFloat("mapIntensity", &mapIntensity, 0.00001f, 100.f);
		...
		for (size_t i = 1; i <= rows; ++i)
			for (size_t j = 1; j <= cols; ++j)
				if (tpSrc[i][j] > 0) {
					...
					mat4 m = glm::translate(vec3(-frameWidth / 2 + (j - 0.5) * cellSize, -frameHeight / 2 + (i - 0.5) * cellSize, 0.f));
					shader.set_mat4("MVP", pv * m);
					greenDotBuffer.draw(GL_TRIANGLES);
				}
